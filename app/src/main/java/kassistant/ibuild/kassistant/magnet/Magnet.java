package kassistant.ibuild.kassistant.magnet;
...
import kassistant.ibuild.kassistant.R;
...
public class Magnet implements View.OnTouchListener {
    ...
    private boolean onlyRightWall = false;
    ...
    private boolean isBackgroundRed = false;
    public static class Builder {
        ...
        public Builder setIsOnlyRightWall(boolean isOnlyRightWall){
            magnet.onlyRightWall = isOnlyRightWall;
            return this;
        }
        public Magnet build() {
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        if (eaten) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                if(mListener!=null) mListener.onIconTouchDown();
                showRemoveView();
    ...
    private void move(float deltaX, float deltaY) {
        ...
        if(shouldFlingAway && onlyRightWall){
            if(isBeingDragged){
                if(mLayoutParams!=null && mRemoveView!=null && mContext!=null){
                    if(Math.abs(mLayoutParams.y - (mContext.getResources().getDisplayMetrics().heightPixels / 2)) < 250){
                        if(!isBackgroundRed){
                            mRemoveView.setShadowBackground(R.drawable.bottom_red_shadow);
                            isBackgroundRed = true;
                        }
                    }
                    else{
                        if(isBackgroundRed){
                            mRemoveView.setShadowBackground(R.drawable.bottom_shadow);
